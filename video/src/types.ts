...
export function isSafeLocalAsset(assetPath: string): boolean {
  return assetPath.length > 0 && !assetPath.startsWith('/') && !assetPath.includes('..') && !assetPath.includes('\\') && !/^[a-z]+:/i.test(assetPath);
}
...
export function validateVideoTemplateDefinition<TInput extends Record<string, unknown>>(
  ...
  if (definition.schemaVersion !== VIDEO_TEMPLATE_SCHEMA_VERSION) throw new TypeError('Unsupported video template schema.');
  if (!/^[A-Za-z0-9-]+$/.test(definition.id)) throw new TypeError(`Invalid composition id "${definition.id}".`);
  if (!Number.isInteger(definition.width) || definition.width <= 0) throw new TypeError('Video width must be a positive integer.');
  if (!Number.isInteger(definition.height) || definition.height <= 0) throw new TypeError('Video height must be a positive integer.');
  if (!Number.isInteger(definition.fps) || definition.fps <= 0) throw new TypeError('Video fps must be a positive integer.');
  if (!Number.isInteger(definition.durationInFrames) || definition.durationInFrames <= 0) throw new TypeError('Video duration must be a positive integer.');
  if (!definition.entryPoint.endsWith('/index.ts')) throw new TypeError('Video entryPoint must identify the package registration module.');
  ...
  for (const font of definition.fonts) {
    if (!font.family.trim() || !font.packageName.startsWith('@fontsource/') || !font.license.trim() || font.weights.length === 0) {
