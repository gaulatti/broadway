...
await assertInputAssets(inputProps, path.join(packageRoot, "public"));
...
try {
  ...
  if (
    ...
    composition.durationInFrames !==
      modoItalianoGiorgiaDefinition.durationInFrames
  ) {
    throw new Error(
      "Selected composition metadata does not match the versioned template definition.",
    );
  }
  ...
  const report = {
    ...
    wallSeconds:
      Number(process.hrtime.bigint() - wallStartedAt) / 1_000_000_000,
