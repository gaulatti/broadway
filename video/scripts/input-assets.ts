...
export async function assertInputAssets(input: ModoItalianoGiorgiaVideoInput, publicDirectory: string): Promise<void> {
  ...
  try {
    ...
    if (!(await stat(assetPath)).isFile()) throw new Error('asset is not a regular file');
  } catch {
    throw new TypeError(`audioAsset "${input.audioAsset}" must resolve to a regular file inside video/public.`);
  }
}
