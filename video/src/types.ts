...
export function assertModoItalianoGiorgiaInput(
  value: unknown,
): asserts value is ModoItalianoGiorgiaVideoInput {
  ...
  if (unknownFields.length > 0) {
    throw new TypeError(
      `Video input has unknown fields: ${unknownFields.join(", ")}.`,
    );
  }
  if (input.schemaVersion !== VIDEO_TEMPLATE_SCHEMA_VERSION) {
    throw new TypeError(
      `Video input schemaVersion must be ${VIDEO_TEMPLATE_SCHEMA_VERSION}.`,
    );
  }
  for (const field of REQUIRED_TEXT_FIELDS) {
    if (typeof input[field] !== "string" || !input[field].trim()) {
      throw new TypeError(
        `Video input field "${field}" must be a non-empty string.`,
      );
    }
    ...
    if (
      text.includes("\n") ||
      text.includes("\r") ||
      Array.from(text).length > TEXT_LIMITS[field]
    ) {
      throw new TypeError(
        `Video input field "${field}" exceeds its title-safe text boundary.`,
      );
    }
  ...
  if (input.audioAsset !== undefined) {
    if (
      typeof input.audioAsset !== "string" ||
      !isSafeLocalAsset(input.audioAsset)
    ) {
      throw new TypeError(
        "audioAsset must be a safe path inside the video public directory.",
      );
    }
  }
}
...
export function validateVideoTemplateDefinition<
  TInput extends Record<string, unknown>,
>(definition: VideoTemplateDefinition<TInput>): void {
  if (definition.schemaVersion !== VIDEO_TEMPLATE_SCHEMA_VERSION)
    throw new TypeError("Unsupported video template schema.");
  if (!/^[A-Za-z0-9-]+$/.test(definition.id))
    throw new TypeError(`Invalid composition id "${definition.id}".`);
  ...
  if (
    !Number.isInteger(definition.durationInFrames) ||
    definition.durationInFrames <= 0
  )
    throw new TypeError("Video duration must be a positive integer.");
  if (!definition.entryPoint.endsWith("/index.ts"))
    throw new TypeError(
      "Video entryPoint must identify the package registration module.",
    );
  if (definition.fonts.length === 0)
    throw new TypeError(
      "A video template must declare at least one local font.",
    );
  for (const font of definition.fonts) {
    if (
      ...
      throw new TypeError(
        `Invalid local font declaration for "${font.family}".`,
      );
    }
  }
